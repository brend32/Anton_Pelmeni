...
screen intro():
    ...
    text _("{color=#e4422d}{size=+20}ДИСКЛЕЙМЕР:{/size}{/color}\nЭто неофициальный фрагмент и его сюжет не имеет никакого отношенния к оригинальной истории и реальным событиям. Также разработчики дали добро на разработку и распространение даной подделки."):
        at alpha(0.5)
        xalign 0.5
        yalign 0.5
        text_align 0.5
...
label anton_pelmeni_start:
    ...
    label .begin:
        ...
        "Помню, в холодильнику лежат пельмени."
    ...
    label .bring_pan:
        $ state.add("pan")
        "Набираем кастрюлку воды. Вот так до средины."
    ...
    label .wait_3:
        if put_pelmeni_time == -1:
            "Думаю, нет."
            "Набирал ледяную воду. Дам её ещё времени."
        elif put_pelmeni_time == 1:
    ...
    label .wait_4:
        if put_pelmeni_time == -1:
            ...
            "Пельмени должны варится 7-9 минут. Тогда можно добится истиного вкуса пельмений."
            "Нужно лишь потерпеть."
            "Пусть ребята ещё полижат."
    ...
    label .after_4():
        if put_pelmeni_time == -1:
            ...
            "Нашы ребята уже всплыли. Видимо не понравилось на дне лежать."
            "Выгледят апетитно."
    ...
    label .end_good:
        ...
        "Вкусные, сочные!"
        "То, что нужно было для хорошого обеда."
